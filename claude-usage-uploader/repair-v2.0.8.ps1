...
$binaryName = 'ClaudeUsageUploader_v2.0.8-win-x64.exe'
...
# ErrorRecord â€” and with $ErrorActionPreference='Stop' (set above) that becomes a
...
# v2.0.8: distinguish a FRESH INSTALL from a REPAIR.
#
...
# register its scheduled task until that form is submitted. The task assertion at
# the end of this script is therefore correct for a repair and wrong for a first
# install, where it would abort a perfectly successful run. Capture the state
# before we touch anything and branch on it at the end.
$hadConfig = Test-Path -LiteralPath (Join-Path $configDir 'config.json') -PathType Leaf
...
Write-Step 'Installing the verified v2.0.8 files without deleting user settings.'
...
if ($hadConfig) {
  Write-Step 'Starting v2.0.8 so it can recreate its supervised background tasks.'
} else {
  Write-Step 'Starting v2.0.8 - it will open your browser so you can register.'
}
Start-Process -FilePath $targetBinary -WorkingDirectory $installDir
Start-Sleep -Seconds 8

if ($hadConfig) {
  # Repair path: this machine was already registered, so the task must come back.
  ...
  if ($LASTEXITCODE -ne 0) {
    throw 'v2.0.8 started, but the background task was not registered. Run this repair as the same Windows user who runs the uploader (not a different admin account).'
  }
  Write-Step 'Repair complete. Configuration was preserved and v2.0.8 is running.'
} else {
  ...
  Write-Host ''
  Write-Step 'Install complete. ONE STEP LEFT:'
  Write-Host '  A page has opened in your browser. Enter your First and Last name,'
  Write-Host '  then click Save. The background service configures itself from there'
  Write-Host '  and starts automatically at every login.'
  Write-Host ''
  Write-Host '  If no browser page appeared, open this file and follow it manually:'
  Write-Host '    install guide.txt (in this folder)'
}
