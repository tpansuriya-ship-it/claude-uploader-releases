...
$binaryName = 'ClaudeUsageUploader_v2.0.9-win-x64.exe'
...
# v2.0.9: distinguish a FRESH INSTALL from a REPAIR.
#
...
Write-Step 'Installing the verified v2.0.9 files without deleting user settings.'
...
if ($hadConfig) {
  Write-Step 'Starting v2.0.9 so it can recreate its supervised background tasks.'
} else {
  Write-Step 'Starting v2.0.9 - it will open your browser so you can register.'
}
...
if ($hadConfig) {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw 'v2.0.9 started, but the background task was not registered. Run this repair as the same Windows user who runs the uploader (not a different admin account).'
  }
  Write-Step 'Repair complete. Configuration was preserved and v2.0.9 is running.'
} else {
